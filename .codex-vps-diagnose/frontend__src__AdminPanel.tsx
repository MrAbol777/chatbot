...
type AdminTab = 'dashboard' | 'users' | 'imageGenerations' | 'videoGenerations' | 'noaFinance' | 'aiRouting' | 'videoPromptProfiles' | 'subscriptions' | 'limits' | 'errors' | 'siteSettings' | 'supervisedOtp' | 'config' | 'audit';
...
const TAB_DESCRIPTIONS: Record<AdminTab, string> = {
  ...
  subscriptions: 'مدیریت پلن‌ها و اشتراک فعال کاربران',
  limits: 'تنظیم سقف پیام و تولید تصویر برای مهمان‌ها و پلن‌ها',
              ...
              {selectedUser.conversations.map((conv) => (
                <details key={conv.conversation_id} className="profile-conversation">
                  ...
                  <div className="admin-note">
                    <strong>عنوان فعلی:</strong> {conv.title || '-'}<br />
                    <strong>عنوان اولیه:</strong> {conv.generated_title || '-'} | <strong>منبع:</strong> {conv.title_source || 'default'}<br />
                    <strong>وضعیت:</strong> {conv.title_generation_status || '-'} | <strong>مدل:</strong> {conv.title_model || '-'} | <strong>نسخه:</strong> {conv.title_generator_version || '-'} | <strong>زمان:</strong> {conv.title_generation_latency_ms ?? '-'}ms<br />
                    <strong>تولید:</strong> {conv.title_generated_at || '-'} | <strong>ویرایش دستی:</strong> {conv.title_manually_updated_at || '-'}
                  </div>
