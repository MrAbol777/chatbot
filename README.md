...
> **وضعیت انتشار: نسخه ۱ (`1.0.0`)** — این README مبنای فعلی محصول برای بستن نسخه اول است. قابلیت‌های شرح‌داده‌شده در این سند، همراه با تنظیمات، migrationها و تست‌های مرتبط، محدوده نسخه ۱ را تشکیل می‌دهند. اصلاحات و توسعه‌های بعدی در شاخه نسخه `1.1` پیگیری می‌شوند.
...
- [تعریف و مرز نسخه ۱](#تعریف-و-مرز-نسخه-۱)
...
نسخه ۱ (`1.0.0`) نسخه پایه و قابل اتکای دانوآ است؛ یعنی هسته محصول، مسیرهای اصلی کاربر، زیرساخت داده، پنل مدیریت، سیستم اعتبار و لایه‌های امنیتی در repository حاضر کنار هم قرار گرفته‌اند و از اینجا به بعد تغییرات باید به‌صورت کنترل‌شده و با حفظ قراردادهای فعلی انجام شوند.

### آنچه در نسخه ۱ بسته شده است

- ورود و ثبت‌نام کاربر با شماره موبایل و OTP، نشست امن و تکمیل پروفایل.
- چت فارسی با streaming، حافظه مکالمه، عنوان‌گذاری و intent routing.
- استودیوی تصویر شامل تولید، ویرایش، تحلیل تصویر، گالری و کنترل سهمیه.
- استودیوی ویدیو شامل job queue، worker، polling، routing provider و ذخیره‌سازی امن.
- سیستم اعتبار نوآ شامل wallet، ledger، reservation، receipt و قیمت‌گذاری سرویس‌ها.
- پنل مدیریت برای کاربران، تنظیمات، routing مدل‌ها، گزارش‌ها، خطاها و audit log.
- رابط RTL واکنش‌گرا، تم‌های `energy` و `calm`، Design System پایه و Health Check.
- ساختار لایه‌ای بک‌اند، migrationهای پایگاه داده، تست‌های اصلی و مسیرهای دیپلوی Docker/PM2/Nginx.

### نکته مهم درباره فعال‌سازی قابلیت‌ها

«بسته‌شدن نسخه ۱» به معنی آماده‌بودن قرارداد و مسیر اجرایی قابلیت‌ها در کد است؛ فعال‌شدن بعضی قابلیت‌ها در هر محیط به provider، کلیدهای API، migration، storage و feature flag وابسته است. به‌طور مشخص، Viana، تولید ویدیو، پرداخت آنلاین و Bale Monitor باید با تنظیمات محیط و readiness check مربوط به خود فعال و اعتبارسنجی شوند.

تصمیم‌های محصولی باز مانند سیاست guardian و consent، moderation بحران، retention، قیمت‌گذاری نهایی، KPI/SLA و provider/fallback نهایی، بخشی از ادعای نسخه ۱ نیستند و باید قبل از تبدیل‌شدن به قرارداد محصول در PRD تصمیم‌گیری شوند. جزئیات وضعیت اجرایی در [`docs/current-state.md`](docs/current-state.md) نگهداری می‌شود.

### مرز نسخه‌های بعدی

نسخه `1.1` برای اصلاحات incremental، بهبود تجربه کاربری، تکمیل adoption سیستم طراحی، پایدارسازی قابلیت‌های وابسته به provider و تصمیم‌های محصولی جدید است. این موارد نباید برای بستن baseline نسخه ۱ به قراردادهای فعلی اضافه شوند؛ هر تغییر ناسازگار با API، schema یا رفتار فعلی باید به نسخه‌ای با تصمیم انتشار جداگانه منتقل شود.

برای ثبت این baseline، پس از بررسی تست‌ها و تغییرات موجود، می‌توان commit نسخه را با پیام زیر انجام داد:

```bash
git add README.md frontend backend deploy docs package.json package-lock.json
git commit -m "docs: close version 1 baseline"
git tag v1.0.0
