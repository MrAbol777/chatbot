...
const { createVideoQuotaService } = require('../src/modules/video-generation/quota/video-quota.service');
...
function fixtureIds() { const requested = String(process.env.METIS_VIDEO_LIVE_TEST_RUN_ID || new Date().toISOString().slice(0, 10).replaceAll('-', '')).toLowerCase(); const runId = requested.replace(/[^a-z0-9_-]/g, '').slice(0, 40); if (!runId) throw new Error('METIS_VIDEO_LIVE_TEST_RUN_ID is invalid.'); return { runId, userId: `metis-live-user-${runId}`, planId: `metis-live-plan-${runId}` }; }
...
async function ensureFixture(db, plansRepository, ids) {
  ...
  const fixturePlan = { id: ids.planId, name: 'Metis Video Live Test (test-only)', icon: '🧪', tagline: 'Local paid-provider fixture only', monthlyPrice: 0, dailyPrice: 0, dailyMessageLimit: 0, dailyImageLimit: 0, hourlyImageLimit: 0, videoLimit: 1, features: ['test-only'], isActive: true, sortOrder: 9999 };
  const current = await plansRepository.getPlanById(ids.planId);
  if (current && (current.name !== fixturePlan.name || current.videoLimit !== 1 || !current.isActive)) throw new Error('Existing test-only plan fixture does not match the live-test contract.');
  if (!current) await plansRepository.upsertPlan(fixturePlan);
  const subscriptions = await plansRepository.readUserSubscriptions();
  const existing = subscriptions.find((item) => String(item.userId) === ids.userId);
  if (existing && (existing.planId !== ids.planId || existing.status !== 'active')) throw new Error('Existing test-only subscription fixture does not match the live-test contract.');
  if (!existing) await plansRepository.writeUserSubscriptions([...subscriptions, { userId: ids.userId, planId: ids.planId, status: 'active', assignedAt: new Date().toISOString(), expiresAt: null, note: 'test-only Metis video live test' }]);
  return { user: ids.userId, plan: ids.planId, subscription: { userId: ids.userId, planId: ids.planId, status: 'active' } };
}
async function usage(db, userId) { const key = new Date().toISOString().slice(0, 7); const [rows] = await db.query('SELECT video_used,video_reserved FROM app_video_usage WHERE user_id=? AND period_key=? LIMIT 1', [userId, key]); return { used: Number(rows[0]?.video_used || 0), reserved: Number(rows[0]?.video_reserved || 0) }; }
async function readLiveJob(db, userId) { const [rows] = await db.query('SELECT id,status,provider_job_id,quota_reservation_id FROM app_video_generations WHERE user_id=? ORDER BY created_at DESC LIMIT 1', [userId]); return rows[0] || null; }
...
async function main() {
  ...
  try {
    ...
    const plansRepository = new PlanRepository(db);
    report.fixture = await ensureFixture(db, plansRepository, ids);
    report.fixture.usage = await usage(db, ids.userId);
    ...
    const service = createVideoGenerationService({ repository: createVideoGenerationRepository(db), quotaService: createVideoQuotaService({ db, plansRepository }), provider, canUseInactiveModel: ({ userId, model: candidate }) => userId === ids.userId && candidate.internal_key === METIS_KLING_V25_TURBO_PRO.internalKey && Number(candidate.is_active) === 0 });
    let job;
    try {
      ...
      report.outcome = 'submit-failed'; report.error = safeError(error); report.internalJobStatus = stored?.status || null; report.reservationStatus = null; report.usage = await usage(db, ids.userId); console.log(JSON.stringify(report, null, 2)); return;
    }
    await new Promise((resolve) => setTimeout(resolve, STATUS_WAIT_MS));
    let status;
    try { status = await provider.getJobStatus(job.provider_job_id); }
    catch (error) { const stored = await readLiveJob(db, ids.userId); report.outcome = 'status-check-failed'; report.error = safeError(error); report.internalJobStatus = stored?.status || null; report.usage = await usage(db, ids.userId); console.log(JSON.stringify(report, null, 2)); return; }
    ...
    else if (normalized === 'failed' || normalized === 'cancelled') { await createVideoQuotaService({ db, plansRepository }).release({ reservationId: job.reservationId, reason: normalized === 'cancelled' ? 'provider_cancelled' : 'provider_failure' }); await db.query("UPDATE app_video_generations SET status=?, provider_status=?, failed_at=NOW(), updated_at=NOW() WHERE id=?", [normalized, normalized === 'failed' ? 'ERROR' : 'CANCELLED', job.id]); report.outcome = normalized === 'failed' ? 'provider-failed' : 'provider-cancelled'; }
    else report.outcome = ['queued', 'submitted', 'processing'].includes(normalized) ? 'provider-pending' : 'provider-unknown-status';
    const stored = await readLiveJob(db, ids.userId); const [reservations] = await db.query('SELECT status FROM app_video_quota_reservations WHERE id=? LIMIT 1', [job.reservationId]); report.internalJobStatus = stored?.status || null; report.reservationStatus = reservations[0]?.status || null; report.usage = await usage(db, ids.userId); console.log(JSON.stringify(report, null, 2));
