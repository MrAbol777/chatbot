...
const { BANANAAI_IMAGE_TO_VIDEO_MODEL_KEY } = require('../src/modules/video-generation/video-model.registry');
...
async function databaseChecks(env) {
  ...
  try {
    ...
    if (!bananaProviders[0] || Number(bananaModels[0]?.total || 0) !== 6 || Number(bananaModels[0]?.active || 0) !== 1 || Number(bananaModels[0]?.public_count || 0) !== 0 || !i2vRoutes[0]) throw new Error('BananaAI Grok seed is incomplete.');
    ...
    return { bananaProviderEnabled: Boolean(bananaProviders[0].enabled), bananaActiveModels: Number(bananaModels[0].active || 0), bananaPublicModels: Number(bananaModels[0].public_count || 0), imageRouteEnabled: Boolean(i2vRoutes[0].enabled), grokImageToVideoPinned: true, promptProfilesReady: true };
...
async function main({ env = process.env } = {}) {
  ...
  if (database.bananaActiveModels !== 1 || database.bananaPublicModels) throw new Error('BananaAI must keep exactly one private Grok model active.');
  if (activationExpected) {
    if (report.featureEnabled !== true || !worker.enabled || !env.BANANAAI_API_KEY || report.bananaResultContract !== 'CONFIGURED' || !providerInputConfigured || !database.bananaProviderEnabled || !database.imageRouteEnabled) {
      throw new Error('Activated BananaAI image-to-video readiness is incomplete.');
    }
  } else if (database.bananaProviderEnabled || database.imageRouteEnabled) {
