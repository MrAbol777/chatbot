...
const normalizePlan = (plan = {}, fallback = {}) => {
  ...
  return {
    ...
    videoLimit: normalizeNullableLimit(plan.videoLimit, fallback.videoLimit ?? null),
...
const fromRow = (row) => {
  ...
  return normalizePlan({
    ...
    videoLimit: row.video_limit == null ? null : Number(row.video_limit),
...
class PlanRepository {
  ...
  async upsertPlan(input, { seed = false } = {}) {
    ...
    await this.db.query(
      `INSERT INTO app_plans
        (id, name, icon, tagline, monthly_price, daily_price, daily_message_limit, daily_image_limit, hourly_image_limit, video_limit, features, is_active, sort_order, created_at, updated_at)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
  ...
  // Video generation is intentionally not granted by the fallback/free plan.
  // A caller must have an active subscription whose assigned plan is active.
  async getVideoPlanForUser(userId) {
    const subscriptions = await this.readUserSubscriptions();
    const activeSubscription = subscriptions.find((item) => String(item.userId) === String(userId) && this.isSubscriptionActive(item));
    if (!activeSubscription?.planId) return null;
    const assignedPlan = await this.getPlanById(activeSubscription.planId);
    return assignedPlan?.isActive ? assignedPlan : null;
  }
