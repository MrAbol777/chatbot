class InputOptimizationRepository {
  ...
  async upsert(record) {
    ...
    await this.db.query(
      `INSERT INTO input_optimizations
       (operation_type, operation_id, conversation_id, turn_id, attempt_id, image_generation_id, user_id,
      ...
      [
        ...
        record.imageGenerationId || null, record.userId || null, record.originalText || '',
  ...
  async listForAdmin({ conversationId = '', status = '', limit = 50 } = {}) {
    ...
    const [rows] = await this.db.query(
      `SELECT id, operation_type, operation_id, conversation_id, turn_id, attempt_id, image_generation_id, user_id,
              original_input, optimized_input, source_language, target_language, ambiguity_level, needs_clarification,
