...
function loadRuntimeConfig(env = process.env) {
  ...
  const conversationTitleKey = pickApiKey([
    { source: 'METIS_CONVERSATION_TITLE_API_KEY', value: env.METIS_CONVERSATION_TITLE_API_KEY },
    { source: 'legacy METIS_CHAT_API_KEY', value: env.METIS_CHAT_API_KEY },
    { source: 'legacy METIS_API_KEY', value: env.METIS_API_KEY }
  ]);
  ...
  const ai = {
    ...
    conversationTitle: {
      enabled: env.CONVERSATION_TITLE_ENABLED !== 'false',
      model: env.CONVERSATION_TITLE_MODEL || 'gemini-2.5-flash-lite',
      temperature: Number.isFinite(Number(env.CONVERSATION_TITLE_TEMPERATURE)) ? Number(env.CONVERSATION_TITLE_TEMPERATURE) : 0,
      timeoutMs: Number.isFinite(Number(env.CONVERSATION_TITLE_TIMEOUT_MS)) ? Number(env.CONVERSATION_TITLE_TIMEOUT_MS) : 6000,
      maxRetries: Number.isFinite(Number(env.CONVERSATION_TITLE_MAX_RETRIES)) ? Number(env.CONVERSATION_TITLE_MAX_RETRIES) : 1,
      maxOutputTokens: Number.isFinite(Number(env.CONVERSATION_TITLE_MAX_OUTPUT_TOKENS)) ? Number(env.CONVERSATION_TITLE_MAX_OUTPUT_TOKENS) : 64,
      maxCharacters: Number.isFinite(Number(env.CONVERSATION_TITLE_MAX_CHARACTERS)) ? Number(env.CONVERSATION_TITLE_MAX_CHARACTERS) : 40,
      version: env.CONVERSATION_TITLE_VERSION || '1',
      apiKey: conversationTitleKey.apiKey,
      apiKeySource: conversationTitleKey.apiKeySource,
      apiKeyFingerprint: conversationTitleKey.apiKeyFingerprint
    }
  };
