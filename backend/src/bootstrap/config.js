...
function loadRuntimeConfig(env = process.env) {
  ...
  const geminiApiKey =
    typeof (env.GEMINI_API_KEY || env.METIS_API_KEY || env.OPENAI_API_KEY) === 'string'
      ? (env.GEMINI_API_KEY || env.METIS_API_KEY || env.OPENAI_API_KEY).trim()
      : '';
  const geminiImageModel = env.GEMINI_IMAGE_MODEL || env.METIS_IMAGE_MODEL || 'gemini-2.5-flash-image';
  const geminiBaseUrl = normalizeBaseUrl(
    env.GEMINI_API_BASE_URL || env.GEMINI_BASE_URL,
