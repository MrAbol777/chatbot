...
function validateSubmit({ prompt, aspectRatio = '1:1', files }) {
  const exactPrompt = typeof prompt === 'string' ? prompt : '';
  const normalizedPrompt = exactPrompt.trim();
  const normalizedAspectRatio = String(aspectRatio || '1:1').trim();
  if (normalizedPrompt.length < 3 || exactPrompt.length > 3_000) {
    throw imageToImageError('IMAGE_TO_IMAGE_PROMPT_INVALID', 'متن ویرایش باید بین ۳ تا ۳۰۰۰ کاراکتر باشد.');
  }
  ...
  return { prompt: exactPrompt, aspectRatio: normalizedAspectRatio };
}
