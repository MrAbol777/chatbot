...
class CharacterWorkspaceRepository {
  ...
  async list(userId) {
    ...
    return rows.map((row) => ({ id: row.workspace_id, title: row.title, status: row.status, createdAt: row.created_at, updatedAt: row.updated_at }));
  }
  ...
  async get(userId, id) {
    ...
    const row = rows[0];
    return { ...safeJson(row.workspace), id: row.workspace_id, title: row.title, scenario: row.scenario, status: row.status, createdAt: row.created_at, updatedAt: row.updated_at };
  }
...
module.exports = { CharacterWorkspaceRepository };
