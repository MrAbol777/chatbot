...
function createVideoResultOrchestrator({ storage, config, logger = null, clock = () => new Date(), faultInjector = null }) {
  ...
  return {
    async store({ job, provider, descriptor, repository, workerId }) {
      ...
      await repository.recordStorageAttempt({ jobId: job.id, workerId });
      try {
        ...
        try {
          const remote = await provider.fetchResultStream(descriptor, { timeoutMs: config.timeoutMs, maxBytes: config.maxBytes, maxRedirects: config.maxRedirects });
        ...
        if (error?.retryable && attempt < config.maxAttempts) {
          ...
          await repository.scheduleStorageRetry({ jobId: job.id, workerId, nextStorageAttemptAt, errorCode: code });
          logger?.info?.({ event: 'video_result_storage_retry', generationId: job.id, errorCode: code });
          return { action: 'storage-retry', errorCode: code, nextStorageAttemptAt };
        }
        await repository.failStorageAndRelease({ jobId: job.id, workerId, errorCode: attempt >= config.maxAttempts ? 'VIDEO_STORAGE_MAX_ATTEMPTS_REACHED' : code });
        logger?.info?.({ event: 'video_result_storage_failed', generationId: job.id, errorCode: code });
