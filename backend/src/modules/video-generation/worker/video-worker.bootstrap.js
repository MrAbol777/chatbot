...
function createVideoWorkerProviderRegistry({ httpClient, env = process.env, storageConfig = null }) {
  ...
  return {
    ...
    bananaai: createBananaAiVideoProvider({ httpClient, baseUrl: env.BANANAAI_BASE_URL || 'https://bananaai.ir', apiKey: env.BANANAAI_API_KEY, proxyUrl: env.BANANAAI_PROXY_URL, forceIpv4: String(env.BANANAAI_FORCE_IPV4 ?? 'true').toLowerCase() !== 'false', requestTimeoutMs: Number(env.BANANAAI_REQUEST_TIMEOUT_MS || 10000), statusTimeoutMs: Number(env.BANANAAI_STATUS_TIMEOUT_MS || 30000), maxTextPromptLength: Number(env.BANANAAI_T2V_MAX_PROMPT_LENGTH || 8000), maxImagePromptLength: Number(env.BANANAAI_I2V_MAX_PROMPT_LENGTH || 2000), resultAllowedHosts: splitList(env.BANANAAI_VIDEO_RESULT_ALLOWED_HOSTS), resultAllowedPorts: storageConfig?.allowedPorts || [443], resultAllowedPathPrefixes: splitList(env.BANANAAI_VIDEO_RESULT_ALLOWED_PATH_PREFIXES), resultConnectTimeoutMs: storageConfig?.connectTimeoutMs, resultHeadersTimeoutMs: storageConfig?.headersTimeoutMs, resultIdleTimeoutMs: storageConfig?.idleTimeoutMs, resultTotalTimeoutMs: storageConfig?.totalTimeoutMs, resultMaxBytes: storageConfig?.maxBytes, resultMaxRedirects: storageConfig?.maxRedirects })
  };
}
