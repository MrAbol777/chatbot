...
function createVideoWorkerRepository(db, { faultInjector, claimProvider = null, noaBillingService } = {}) {
  ...
  return {
    ...
    recordStorageAttempt: async ({ jobId, workerId }) => { const [result] = await db.query("UPDATE app_video_generations SET storage_attempts=storage_attempts+1, updated_at=NOW() WHERE id=? AND status='storing' AND worker_lease_owner=? AND worker_lease_until>NOW()", [jobId, safeText(workerId, 191)]); return result.affectedRows === 1; },
    scheduleStorageRetry: async ({ jobId, workerId, nextStorageAttemptAt, errorCode }) => { const [result] = await db.query("UPDATE app_video_generations SET next_storage_attempt_at=?, storage_safe_error_code=?, storage_safe_error_message='دریافت نتیجه ویدیو موقتاً ناموفق بود.', worker_lease_owner=NULL, worker_lease_until=NULL, updated_at=NOW() WHERE id=? AND status='storing' AND worker_lease_owner=? AND worker_lease_until>NOW()", [nextStorageAttemptAt, safeText(errorCode, 100), jobId, safeText(workerId, 191)]); return result.affectedRows === 1; },
