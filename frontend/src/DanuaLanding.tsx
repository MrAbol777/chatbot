...
const features: Array<{
  ...
  },
  {
    icon: 'studio',
    title: 'استودیو',
    text: 'ساخت تصویر و ویدیو با هوش مصنوعی',
    tone: 'blue'
  },
  {
    icon: 'story',
      ...
      <section id="features" className="danua-section">
        ...
        <div className="danua-feature-grid">
          {features.map((feature) => {
            const isStudio = feature.icon === 'studio';
            return (
            <Card
              key={feature.title}
              className={`danua-feature-card danua-tone-${feature.tone} ${isStudio ? 'danua-feature-card--interactive' : ''}`}
              padding="lg"
              role={isStudio ? 'button' : undefined}
              tabIndex={isStudio ? 0 : undefined}
              onClick={isStudio ? navigateToStudio : undefined}
              onKeyDown={isStudio ? (event) => {
                if (event.key === 'Enter' || event.key === ' ') {
                  event.preventDefault();
                  navigateToStudio();
                }
              } : undefined}
              aria-label={isStudio ? 'باز کردن استودیو' : undefined}
            >
