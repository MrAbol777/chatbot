...
export type VideoCapabilityOption = {
  ...
  allowedResolutions: string[];
  maxPromptLength: number | null;
  providerPromptMaxLength?: number | null;
  supportsNegativePrompt?: boolean;
  supportsAudio?: boolean;
};
...
export type VideoSubmitInput = { mode: VideoGenerationMode | 'image_to_video' | 'text_to_video'; styleKey: string; mediaId?: string; prompt: string; aspectRatio: string; duration: string; resolution: string; generateAudio?: boolean };
...
export type VideoGenerationErrorCode =
  ...
  | 'VIDEO_INVALID_SETTINGS' | 'VIDEO_GENERATION_PROMPT_TOO_LONG' | 'VIDEO_GENERATION_OPTIONS_NOT_ALLOWED' | 'VIDEO_GENERATION_IDEMPOTENCY_CONFLICT'
  | 'VIDEO_PROMPT_COMPILER_UNAVAILABLE'
