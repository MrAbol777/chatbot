...
describe('VideoPromptProfilesAdmin',()=>{
  ...
  it('shows full prompt only in the authorized Admin feature and lists immutable versions',async()=>{render(<VideoPromptProfilesAdmin/>);expect(await screen.findByDisplayValue('متن کامل مرجع')).toBeInTheDocument();expect(await screen.findByText('2')).toBeInTheDocument();expect(screen.getByText(/Version جدید/)).toBeInTheDocument();});
  it('previews compilation locally through the Admin endpoint',async()=>{const user=userEvent.setup();render(<VideoPromptProfilesAdmin/>);await screen.findByDisplayValue('متن کامل مرجع');await user.click(screen.getByRole('button',{name:'Compile بدون Provider'}));await waitFor(()=>expect(screen.getByText('[[NON-NEGOTIABLE RULES]] identity')).toBeInTheDocument());const serialized=document.body.textContent||'';expect(serialized).not.toMatch(/api key|bearer|provider task/i);});
});
