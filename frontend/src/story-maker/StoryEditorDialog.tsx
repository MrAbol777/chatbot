...
const sceneFields: Array<{ key: SceneField; label: string; rows?: number }> = [
  ...
  { key: 'action', label: 'اتفاق', rows: 3 }, { key: 'dialogue', label: 'دیالوگ', rows: 3 }, { key: 'narration', label: 'نریشن' }, { key: 'sound', label: 'صدا و موسیقی' },
  { key: 'continuity', label: 'پیوستگی' }, { key: 'transition', label: 'برش بعدی' }, { key: 'imagePrompt', label: 'پرامپت تصویر/ویدیو', rows: 3 }
];
...
export default function StoryEditorDialog({ open, story, expectedScenes, versions, onClose, onSaved, onRestore }: Props) {
  ...
  return <Dialog open={open} title="ویرایش سناریو" onClose={onClose} showFooter={false} panelClassName="story-editor-dialog">
    {!editableStory ? <div className="story-editor-empty"><Icon name="alert-triangle" size={22} aria-hidden="true" /><p>این سناریو داده‌ی قابل‌ویرایش ندارد.</p></div> : <div className="story-editor" dir="rtl">
      ...
      {tab === 'ai' ? <section id="story-editor-ai-panel" role="tabpanel" aria-labelledby="story-editor-ai-tab" className="story-editor__ai"><div className="story-editor__intro"><span><Icon name="sparkle" size={19} aria-hidden="true" /></span><div><strong>چی را عوض کنیم؟</strong><p>دانوآ فقط همان بخشی را که انتخاب می‌کنی تغییر می‌دهد و بقیه‌ی داستان را نگه می‌دارد.</p></div></div><label htmlFor="story-editor-request">درخواست ویرایش<textarea id="story-editor-request" value={request} onChange={(event) => setRequest(event.target.value.slice(0, 500))} placeholder="مثلاً صحنه را خنده‌دارتر کن و دیالوگ پوفی کوتاه‌تر باشد." maxLength={500} /></label><div className="story-editor__scope"><span>کدام بخش؟</span><div role="radiogroup" aria-label="محدوده‌ی ویرایش"><button type="button" role="radio" aria-checked={targetScene === null} className={targetScene === null ? 'is-selected' : ''} onClick={() => setTargetScene(null)}>همه‌ی داستان</button>{editableStory.scenes.map((scene) => <button key={scene.number} type="button" role="radio" aria-checked={targetScene === scene.number} className={targetScene === scene.number ? 'is-selected' : ''} onClick={() => setTargetScene(scene.number)}>صحنه {scene.number}</button>)}</div></div><Button type="button" loading={busy} onClick={() => void saveWithAi()} endIcon={<Icon name="sparkle" size={17} aria-hidden="true" />}>اعمال تغییر با دانوآ</Button></section> : <section id="story-editor-manual-panel" role="tabpanel" aria-labelledby="story-editor-manual-tab" className="story-editor__manual"><div className="story-editor__manual-grid"><label>عنوان<input value={editableStory.title} onChange={(event) => updateStory('title', event.target.value)} /></label><label>لاگ‌لاین<textarea value={editableStory.logline} onChange={(event) => updateStory('logline', event.target.value)} /></label><label>پیام داستان<textarea value={editableStory.message} onChange={(event) => updateStory('message', event.target.value)} /></label><label>سبک تصویری<input value={editableStory.visualStyle} onChange={(event) => updateStory('visualStyle', event.target.value)} /></label><label>دنیای داستان<textarea value={editableStory.world} onChange={(event) => updateStory('world', event.target.value)} /></label><label>پایان<textarea value={editableStory.ending} onChange={(event) => updateStory('ending', event.target.value)} /></label></div><details className="story-editor__beats"><summary>نقشه‌ی داستان</summary>{(['setup', 'goal', 'obstacle', 'climax', 'resolution'] as Array<keyof StoryScenario['storyBeats']>).map((key) => <label key={key}>{({ setup: 'شروع', goal: 'هدف', obstacle: 'مانع', climax: 'نقطه‌ی اوج', resolution: 'حل ماجرا' }[key])}<textarea value={editableStory.storyBeats[key]} onChange={(event) => updateBeat(key, event.target.value)} /></label>)}</details><div className="story-editor__characters">{editableStory.characters.map((character, index) => <details key={`${character.name}-${index}`}><summary>شخصیت: {character.name || 'بدون نام'}</summary>{(['name', 'description', 'goal', 'voiceStyle'] as Array<keyof StoryScenario['characters'][number]>).map((key) => <label key={key}>{({ name: 'نام', description: 'ظاهر و ویژگی', goal: 'هدف', voiceStyle: 'لحن حرف‌زدن' }[key])}{key === 'name' ? <input value={character[key]} onChange={(event) => updateCharacter(index, key, event.target.value)} /> : <textarea value={character[key]} onChange={(event) => updateCharacter(index, key, event.target.value)} />}</label>)}</details>)}</div><div className="story-editor__scenes">{editableStory.scenes.map((scene, index) => <details key={scene.number}><summary>صحنه {scene.number}: {scene.title || 'بدون عنوان'}</summary><label>عنوان صحنه<input value={scene.title} onChange={(event) => updateScene(index, 'title', event.target.value)} /></label>{sceneFields.map((field) => <label key={field.key}>{field.label}<textarea rows={field.rows || 2} value={scene[field.key]} onChange={(event) => updateScene(index, field.key, event.target.value)} /></label>)}</details>)}</div><Button type="button" loading={busy} onClick={() => void saveManual()}><Icon name="check" size={17} aria-hidden="true" /> ذخیره و بررسی کیفیت</Button></section>}
